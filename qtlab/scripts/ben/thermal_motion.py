...
dualsmb = False#True
dfreq = False #7.004 #MHz
external_lock =False# True # if false it simply uses the dfreq and sets itself to internal
'''
drivemode:
...
#set frequencies
RF_start_fixed=302.397#302.5 - 0.5 #295.7 -(1.92/2)#prev. 279-30 #MHz #res 1 at 136Mhz 2 at 306MHz
RF_stop_fixed =302.685#302.5 + 0.8 #295.7 +(1.92/2) #get just enough for the dc component #prev. 279+30 #MHz
RF_step=0.002#0.0125#0.4 #MHz
...
set_fixed_values(yvarname,
                 ...
                 logtimeconstant = log10(1.2e-6), #1.2us  -6.1,#10us timeconstant #-3, #-6 has maximised noise value..
                 Vdd_fixed = 0.4,
                 Vattplus= 0,
                 Vattctrl= 0)

#set oscilloscope numer of points
Notimesteps = 50000 #how many points in trace
scale = 3600 # scale of the lockin output for X,Y,R,Theta
settlingtime = 0.1 #10*timeconstant #settlingtime
averages = 4000 # simply set the number of averages
pulse_period =143#1437#7#237 #us
pulse_width = 0#143#7#43 #us
HFoscil.set_time_scale(50e-6) #sec/div
#HFoscil.set_trigger_pos((-pulse_width-20)*10e-6)#set the trigger position 
...
for zvar in linspace(z_start,z_stop,Nozsteps):
    ...
    for yvar in linspace(y_start,y_stop,Noysteps):
	    ...
	    #get_avg_trace commands waits until averages are done.
            ch2 = HFoscil.get_avg_trace(2,Notimesteps) #X values
            ch3 = HFoscil.get_avg_trace(3,Notimesteps) #Y values
