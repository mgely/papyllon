...
#set frequencies
RF_start_fixed=302.05#prev. 279-30 #MHz #res 1 at 136Mhz 2 at 306MHz
RF_stop_fixed =302.7 #prev. 279+30 #MHz
RF_step=0.002#0.4 #MHz
...
settlingtime = 2*timeconstant #overwrite settlingtime (instead of *10 try *1)
filterorder = 8 #18db/oct #8 use 48dB/Oct
scale = 7000
pulse_period = 1000#1437#7#237 #us
pulse_width  = 1000#7#43 #us
'''
...
if (readoutmode == 'HFlockin') | (readoutmode == 'oscilloscope') | (readoutmode == 'HFoscil'):
    ...
    HFlockin.set_input_range1(0.01) #V
...
if readoutmode == 'HFoscil':
    ...
    if recordxyoscil:
        ...
        HFoscil.set_voltage_position3(3)
        HFoscil.set_voltage_scale4(0.20)
        HFoscil.set_voltage_position4(-2)
...
if readoutmode=='spectrum_analyzer':
    ...
    if readoutmode == 'HFlockin':
        ...
        data.add_value('(R)Channel 1 voltage (V)') #R
        data.add_value('(Pulse)Channel 2 voltage (V)') #pulse
        if recordxyoscil:
            data.add_value('(X)Channel 3 voltage (V)') #X
            data.add_value('(Y)Channel 4 voltage (V)') #Y
            
        data.add_value('DC (A)')
        data.add_value('Temperature')
        
data.create_file()
data.copy_file('frequency_trace_v07.py')
        ...
        for vg in vg_array:
		    ...
		    smb.set_RF_frequency(f)
                    ...
                    if (readoutmode == 'spectrum_analyzer'):
                        ...
                        #time_array, amplitude_array = HFoscil.get_trace(2)
                        ...
                        #calculate the R array from X and Y  R=sqrt(x^2 +y^2)
                        time_array = time3_array
                        amplitude_array = sqrt(amplitude3_array*amplitude3_array + amplitude4_array*amplitude4_array)
                    ...
                    if recorddceachpoint:
                        if readoutmode == 'oscilloscope':
                            ...
                            #i_array=eval(vm.query('FETCH?'))/current_gain*ones(len(time_array))
                            i_array[counter]=eval(vm.query('FETCH?'))/current_gain
                            print 'readoutmode = oscilloscope selected'
                        else:
