...
mvar_1 = 'keithley1'  #Use intrument to read: FSV,keithley1,PNA_single,Adwin,FSV, HFLockin_2smbs, FieldFox
...
#'RF1' [MHz],'RFp1' [dBm]
#'FF_pow',
#'Tc' (time constant [10**x])
#clear res sig: Vg[2.36:2.725] RF[205.86:208.02]
#LO (vg_Pow min is about 7dBm)

#FSV Hemt out (2GHz max)

xvar = 'Vg'#'Frequency [MHz]'    
x_start= -4
x_stop = 4
x_resolution = 0.001 #min: 0.02 & -13dBm 
x_pt   = int(abs((x_stop-x_start)/x_resolution) +1)
x_list = np.linspace(x_start,x_stop,x_pt)
x_sleep = 0.2 #wait before taking the x-sweep 20ms is perfect for Vb

yvar = 'none'
y_start = -0.010
y_stop  = 0.010
y_resolution = 0.01
y_pt   = 1#int(abs((y_stop-y_start)/y_resolution) +1)
...
smb1.set_RF_power(21)
smb1.set_RF_frequency(150)
smb1.set_RF_state(0) #activate 1
#SMB2 is connected to Vb AC in
smb2.set_RF_power(-20)
smb2.set_RF_frequency(8.3)
smb2.set_RF_state(0) #activate 1

#sweep('Vs', 0.29) #activate Hemt
...
print 'estimate req. time:',0.005*x_pt*y_pt*z_pt/3600, 'hours'
