...
#set x variable
xvarname = 'vgate' # 'vgate' or 'RF_power' or 'f_LO' or 'Vdd' or 'tc'
...
for zvar in linspace(z_start,z_stop,Nozsteps):
    ...
    for yvar in linspace(y_start,y_stop,Noysteps):
        ...
        for xvar in linspace(x_start,x_stop,Noxsteps):
            ...
            data.add_data_point(xvar,yvar,zvar,i)

            if Noxsteps > 20: #print sweep number
                if counter%int(((x_stop-x_start)/x_step+1)/20)==0:
                    print coordinate_names[xvarname] + ': %s' % xvar
                counter=counter+1

        data.new_block()
        spyview_process(data,x_start,x_stop,y_start,y_stop,zvar)    
