...
#hours_time=int(estimate_time)
#minutes_time=int((estimate_time%1)*60)
print 'First time estimate in hours: ', int(estimate_time),':',int((estimate_time%1)*60)

#set fixed values
if (xvarname != 'vgatefine') & (yvarname != 'vgatefine') & (zvarname != 'vgatefine'):
    vgatefine_fixed = -0.76 #V
...
if measurevoltage:
    vm2.write('*RST')
    vm2.set_nplc(1) #sets measuremnt time to n powerline cycle by keithley  1 = 20ms
    vm2.set_display(0)                               #turn display off
    vm2.write('SENSe:FUNCtion "VOLTage:DC"')
    vm2.write(':FORM:ELEM READ') #just getting the values nothing else.. :)
    vm2.write('INITiate:CONTinuous OFF;:ABORt')      #vm.set_trigger_continuous(False)
    vm2.write('SYSTem:AZERo:STATe OFF')               #Turn autozero off for speed (will result in voltage offsets over time!!)
    vm2.write('SENSe:VOLTage:DC:AVERage:STATe OFF')  #Turn off filter for speed
    vm2.write('SENSe:VOLTage:DC:RANGe 1')          #give it a fixed range to max speed
    vm2.write('TRIG:DEL:AUTO OFF')                   #set triger delay to manual
    vm2.write('TRIG:DEL 0')                          #TRIGger:DELay to 0 sec
    vm2.write('TRIGger:COUNt 1')
    vm2.get_all()
