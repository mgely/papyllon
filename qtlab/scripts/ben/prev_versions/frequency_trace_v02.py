...
execfile('frequency_trace_routines2.py')
...
data.create_file()
data.copy_file('frequency_trace_v02.py')
data.copy_file('frequency_trace_routines.py')
...
for zvar in linspace(z_start,z_stop,Nozsteps):
    ...
    for yvar in linspace(y_start,y_stop,Noysteps):
        ...
        if (readoutmode == 'oscilloscope')|(readoutmode == 'HFoscil'):
            spyview_process(data,NoRFsteps,min(time_array),min(time_array),Noysteps,y_start,y_stop,Nozsteps,z_start,zvar)
    ...
    if (readoutmode != 'oscilloscope') & (readoutmode != 'HFoscil'):
        spyview_process(data,NoRFsteps,RF_start_fixed,RF_stop_fixed,Noysteps,y_start,y_stop,Nozsteps,z_start,zvar)
