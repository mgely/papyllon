...
powerlist=np.linspace(float(stop_power),float(start_power),3)
...
base_path = 'D:\\data\\Sal\\LETO11_V10mK\\'
...
#Set up data
filename = 'bf_single_tone_PNA_3D_power_quick_scan'
...
data.copy_file('bf_single_tone_PNA_3D_power_quick_scan.py')
...
for gate in gatelist:
    ...
    ##        qt.msleep(sweeptime)
        ...
        data.add_data_point(flist, list(magv*ones(len(flist))),list(gate*ones(len(flist))),trace[0], tr2, trace[1])

        data.new_block()
        spyview_process(data,start_freq,stop_freq,stop_power, start_power,gate,newoutermostblockval=new_outermostblockval_flag)
