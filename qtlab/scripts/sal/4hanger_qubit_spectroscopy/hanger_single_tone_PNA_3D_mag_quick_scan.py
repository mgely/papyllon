...
#Set up data
filename = 'hanger_single_tone_PNA_3D_mag_scan'
...
data.copy_file('hanger_single_tone_PNA_3D_mag_quick_scan.py')
...
for itera in iteration:
    ...
    for magv in magvlist:
        ...
        data.add_data_point(flist, list(magv*ones(len(flist))),list(itera*ones(len(flist))),trace[0], tr2, np.unwrap(trace[1]))

        data.new_block()
        spyview_process(data,start_freq,stop_freq,stop_magv, start_magv,itera,newoutermostblockval=new_outermostblockval_flag)
