...
stop_gate  = 2.25
gate_points=81
my_gatelist=np.linspace(float(start_gate),float(stop_gate),gate_points)
gatelist=my_gatelist
...
data.add_coordinate('magv (V_adwin)')
...
std_bias_error=150e3

min_chi_list=[]

cannot_find_chi_flag=False
...
for cav_power in cav_power_list:
    ...
    for magv in magvlist:
        ...
        print 'Line:', str(line_index), '/', len(magvlist), '\t magnetic field: ', str(magv)

        #set magv
        adwin.set_DAC_2(magv)        #check if it corresponds
        #wait
        qt.msleep(0.2)
        print 'do gatescan to measure minimal chi for this setting'
        my_chi = gate_scan(gate_list=my_gatelist)[2]
        min_chi_list.append(my_chi)
        bias_error = max(std_bias_error,my_chi*0.2)
        print 'min chi found at: ' + str(my_chi)
        ...
        for segment in probe_segments:
            ...
            while(segment_unfinished):
                ...
                if(check_bias(chi=my_chi+bias_error, error=bias_error)):
                    ...
                    bias_ok = check_bias(chi=my_chi+bias_error, error=bias_error)      #check if the bias was still ok.
                    if(bias_ok):
                    ...
                    if(find_balanced_bias(chi=my_chi+0.5*bias_error,error=bias_error, center_gate_index=gatelist.searchsorted(adwin.get_DAC_1()))):
                        print 'balanced bias found'
                        bias_found=True
                    else:
                        print 'find bias traditionally'
                        bias_found=find_bias(chi=my_chi+0.5*bias_error,error=bias_error)
                        if(bias_found==False):
                            my_chi = my_chi + .1e6
                        

        #store qt lab data
        my_magv_dummy = np.linspace(magv,magv,len(probe_list))
        cav_power_dummy = np.linspace(cav_power,cav_power,len(probe_list))
        data.add_data_point(probe_list, my_magv_dummy, cav_power_dummy, probe_data[0],np.unwrap(probe_data[1]))
        data.new_block()

        #reset variables
        probe_data=[[],[]]
        spyview_process(data,start_probe,stop_probe,stop_cav_power,start_cav_power,magv,newoutermostblockval=new_outermostblockval_flag)
        new_outermostblockval_flag=False
