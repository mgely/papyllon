...
#Set up data
filename = 'gate_quick_scan_biasing'
...
data.create_file(datadirs=path)
data.copy_file('gate_quick_scan_biasing.py')
...
for magv in magvlist:
    ...
    for gate in gatelist:
        ...
        while(abs(adwin.get_DAC_1()-gate)>0.01):
            qt.msleep(.1)
        ...
        print 'gate' + str(gate) + 'peak' + str(peak)
        if(peak<-4e5):
            print 'we reached bias point break out of for loop'
            break
        else:
            print 'do two tone spectroscopy'
            break_flag = True

    if(break_flag):    
        break
else:
    print "Let's check stability while smoking a sigaret"
    abort_bias()
