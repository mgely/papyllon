...
pna.w("CALC:MARK:REF ON")
pna.w("CALC:MARK:REF:X " + str(w_bare))
pna.w("CALC:MARK1 ON")
pna.w("CALC:MARK1:FUNC MAX")
pna.w("CALC:MARK1:FUNC:TRAC ON")
pna.w("CALC:MARK1:DELT ON")
...
#Set up data
filename = 'quick_gate_scan_qubit_above'
...
data.copy_file('quick_gate_scan_qubit_above.py')
...
for magv in magvlist:
    ...
    print 'frame: ', str(frame_count), '/', str(len(magvlist)), '\t', time()-tframe, '[s]', 'estimated ready at:', localtime(tstart+exp_time)[3], ':', localtime(tstart+exp_time)[4], ':', localtime(tstart+exp_time)[5]
    ...
    for gate in gatelist:
        ...
        peak_list.append([magv,gate,pna.q("CALC:MARK:X?")])
    ##    
    ##    
    ##    for i in ave_list:
    ##        print 'sweep', i        
    ...
    ##        qt.msleep(sweeptime)
        ...
        data.add_data_point(flist, np.linspace(gate,gate,len(flist)),np.linspace(magv,magv,len(flist)),trace[0], tr2, trace[1])
