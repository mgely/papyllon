...
def check_bias(continuous=False, error=150e3,chi=-900e3):
    '''
    ...
    print 'Gate \t' + str(round(adwin.get_DAC_1(),2)) +'\t Starkshift: \t' + to_MHz(peak) + '\t Chi_target ' + to_MHz(chi) + '\t error: ' + to_kHz(peak-chi) + ' kHz'
...
def find_balanced_bias(continuous=False,error=150e3,chi=-900e3, center_gate_index=40):
    '''
...
def find_bias(continuous=False,error=150e3,chi=-900e3):
    '''
    Function to find the gate Voltage such that cavity stark shifted away from the
    bare cavity. 
    '''
    pna.w("SENS1:SWE:MODE HOLD")
    pna.w("INIT:CONT OFF")
