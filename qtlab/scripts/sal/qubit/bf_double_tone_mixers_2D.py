...
#Measurement script that does double tone qubit spectroscopy using the ADwin
#as a DAC to either sweep magnetic field of gate voltage, and the mixers with
#the Agilent digitizer.
...
#implement TTL Lock step between SMB & Agilent

# for now SMB in sweepmode takes about 6ms to step a frequency & 2ms in listmode
...
if 'dig' not in instlist:
    dig = qt.instruments.create('dig', 'Agilent_L4532A', address='TCPIP::192.168.1.51::INSTR')
...
#Set up data
filename = 'bf_double_tone_mixers_2D'
...
data.add_coordinate('Samples (numbers)')

data.add_value('I (V)')
data.add_value('Q (V)')

#data.add_value('R (V)')
#data.add_value('Phase (V)')
...
data.copy_file('bf_double_tone_mixers_2D.py')
...
#dig.set_Config_channel_1('.5,DC,LP_20_KHZ')
#dig.set_Config_channel_2('.5,DC,LP_20_KHZ')


sampling_rate=20000 #kSample/s
dig.set_Rate(sampling_rate)
dig.set_Records(f_points)
dig.set_Samples_per_Record(400)

dig.set_Trig_arm('IMM')        #arm just before start
dig.set_Trig_source('EXT')
dig.set_Trig_in_slope('NEG')

#INIT SMB

smb.set_start_frequency(100)
smb.set_stop_frequency(1000)
smb.set_dwell_time(100) #in ms should be acquisition time + 6ms
smb.set_sweep_points(f_points)
smb.set_RF_state(False)
#smb.set_Sweep_state(True)
#run a test sweep with RF off
#smb.execute_sweep()
