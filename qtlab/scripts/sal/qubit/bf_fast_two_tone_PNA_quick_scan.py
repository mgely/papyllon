...
start_p = 5.1e9
stop_p = 6e9
p_points = int(stop_p-start_p)/2e6

print 'p_points', p_points

magvlist=np.linspace(float(magv_bias),float(magv_bias),20)


flist=np.linspace(float(start_freq),float(stop_freq),f_points)
plist=np.linspace(float(start_p),float(stop_p),p_points)
...
#Set up data
filename = 'bf_fast_two_tone_PNA_quick_scan'
...
data.copy_file('bf_fast_two_tone_PNA_quick_scan.py')
