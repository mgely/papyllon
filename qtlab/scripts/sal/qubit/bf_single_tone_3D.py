...
gatvlist=np.linspace(float(start_gatvoltage),float(stop_gatvoltage),gatv_points)
...
for magv in magvlist:
    ...
    for gatv in gatvlist:
        #Set Voltage Gate
        adwin.set_DAC_1(gatv)
        ...
        #take trace
        trace=fsv.get_trace()
        data.add_data_point(flist, list(magv*ones(len(flist))),list(gatv*ones(len(flist))),trace)
        data.new_block()

    data.new_block()
    spyview_process(data,start_frequency,stop_frequency,start_magvoltage,stop_magvoltage,gatv)
