...
if 'adwin' not in instlist:
    adwin = qt.instruments.create('adwin', 'ADwin_DAC_sal',address=0x255)
...
pna.w("CALC:MARK ON")
pna.w("CALC:MARK:FUNC MAX")
pna.w("CALC:MARK:FUNC:TRAC ON")
...
base_path = 'D:\\data\\Sal\\LETO10B_300mK\\'
...
#Set up data
filename = 'bf_single_tone_PNA_3D_gate_scan'
...
data.copy_file('bf_single_tone_PNA_3D_gate_quick_scan.py')
...
for magv in magvlist:
    ...
    ##        qt.msleep(sweeptime)
        ...
        data.add_data_point(flist, np.linspace(gate,gate,len(flist)),np.linspace(magv,magv,len(flist)),trace[0], tr2, np.unwrap(trace[1]))
