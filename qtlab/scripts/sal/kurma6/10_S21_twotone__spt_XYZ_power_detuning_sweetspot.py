...
for Z in Z_list:
    ...
    for Y in Y_list:
        ...
        f_cav = measure_cav()
        pna_autoscale()
        # pna_set_2tone_cw_f(f_cav+Y*1e3)
        ...
        reim = pna.fetch_data(channel=2,polar=False)
        formatted_data=pna.fetch_formatted_data(channel=2)

        data.add_data_point(X_list, list(Y*ones(len(X_list))),list(Z*ones(len(X_list))),formatted_data,s21[0],np.unwrap(s21[1]),reim[0],reim[1],list(f_cav*ones(len(X_list))))
        data.new_block()
