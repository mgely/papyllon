...
med.set_device('OFHCCopperCavity')
med.set_setup('RT')
med.set_user('Guoji')
...
####Settings:
# new OFHC copper cavity (rectangular)
# directly coupled to VNA
...
for pw in pow_list:
    ...
    if -30<=pw<-20:
        ...
        pna.set_resolution_bandwidth(2000)
    elif -15<=pw<-10:
        pna.set_resolution_bandwidth(3000)
    elif -10<=pw-5:
        pna.set_resolution_bandwidth(5000)
    elif -5<=pw<10:
        pna.set_resolution_bandwidth(1000)
    elif 10<=pw<=30:
        pna.set_resolution_bandwidth(1000)
...
#run execfile('bf-PNA_trace_powerSweep.py') to start
