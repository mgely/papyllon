    ...
    for Y in Y_list:
        ...
        if current_running==True:
            ...
            if(curr_state):
                ...
                data.add_data_point(X_list, list(Y*ones(len(X_list))),list(Z*ones(len(X_list))),list(current*ones(len(X_list))))#,trace[0], tr2, np.unwrap(trace[1]),trace[1],list(current*ones(len(X_list))))
                data.new_block()
                ...
                data.add_data_point(X_list, list(Y*ones(len(X_list))),list(Z*ones(len(X_list))),list(current*ones(len(X_list))))#,list(noise_polar0*ones(len(X_list))),
                                    #list(noise_f*ones(len(X_list))), list(noise_polar1*ones(len(X_list))),list(noise_polar1*ones(len(X_list))),
                                    #list(current*ones(len(X_list))))
                data.new_block()
                spyview_process(data,X_start,X_stop,Y_stop, Y_start,Z,newoutermostblockval=new_outermostblockval_flag)
                new_outermostblockval_flag=False
                qt.msleep(0.01) #wait 10 usec so save etc
