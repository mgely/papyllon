...
data.copy_file('OMIT_OMIA_up_dn_seq1.py')
...
for freq in freq_list:
    ...
    data.add_data_point(freq_dummy,comb_f_list,trace1[0],trace1[1],trace2[0],trace2[1])
    data.new_block()
    spyview_process(data,1,2*probe_pt,freq)
    qt.msleep(0.01)

#sweep_time = float(pna.q("SENS1:SWE:TIME?"))
data.close_file()
qt.mend()
execfile('OMIT_OMIA_up_dn.py')
