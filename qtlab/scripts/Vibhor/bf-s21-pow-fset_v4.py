...
hanger_idx=np.array([5])*GHz
hanger_span = 2000*MHz #this is the span for each cent. freq.
f1_pts = 5000
f1_bw = 3000 #HZ         #float(f1_stop-f1_start)/float(f1_pts) #500e3#1e9/180e3 #expected q factor
...
for f1_power in pow_list:
    ...
    for f_idx in hanger_idx:
        ...
        data.add_data_point(power_list,f_list,fidx_list,trace[0],trace[1]) #for S21 vs power only
        data.new_block()
