...
base_path = 'D:\\data\\Marnix\\OMIT_1\\'
now=localtime()
...
data.copy_file('bf-PNA_trace_multi.py')
...
f1_pts = 501
f1_bw = 150 #HZ
hanger_f0=5900.5406*MHz # center frequency
hanger_span=30*kHz # Span of measurement
...
for i in range(1,loop_count):
    print i
    pna.sweep()
    dummy_index=np.linspace(i,i,f1_pts)
    trace= pna.fetch_data(polar=True)
    trace2= pna.fetch_data(polar=False)
    data.add_data_point(dummy_index,f_list,trace[0],trace[1],trace2[0],trace2[1])
    data.new_block()
    spyview_process(data,0,1,i)
