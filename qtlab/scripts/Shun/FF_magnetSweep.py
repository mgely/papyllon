...
data.add_coordinate('Voltage [uv]')
data.add_value('S21 [abs]')
data.add_value('S21 [rad]')

#data.create_file()
data.create_file(name=filename, datadirs='D:\\data\\Shun\\ParaAmp1\\20140110\\100SHzBW')
data.copy_file('FF_magnetSweep.py')
...
ff.write('SOUR:POW '+str(probe_power)+'\n')
...
for v in v_list:
    print 'current voltage '+str(v)+' Volt'
    adwin.set_DAC_2(v)
    qt.msleep(2)
    #setting tarce 1
    ff.write('INIT \r')
    ...
    v_dummy=np.linspace(v,v,len(f_list))
    data.add_data_point(v_dummy,f_list,trace_mlog,np.unwrap(np.deg2rad(trace_phase),np.pi))
    data.new_block()
    spyview_process(data,f_start,f_stop,v)
    qt.msleep(0.01)
data.close_file()
qt.mend()
