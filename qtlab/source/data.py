...
class DateTimeGenerator:
    ...
    def create_data_dir(self, datadir, name=None, ts=None, datesubdir=False, timesubdir=False):
        '''
    ...
    def new_filename(self, data_obj, datadirs):
...
#        tstr = time.strftime('%H%M%S', data_obj._localtime)
        dir = self.create_data_dir(datadirs, name=data_obj._name,
                ts=data_obj._localtime)

        filename = '%s_%s.dat' % (data_obj._timecode, data_obj._name)
...
### File writing
    ...
    def create_file(self, name=None, datadirs='D:\\data\\', filepath=None, settings_file=True):
        '''
        ...
        #create a timecode
        timecode = time.strftime('%Y%m%d%H%M%S', self._localtime)
        ...
        if filepath is None:
            filepath = self._filename_generator.new_filename(self,datadirs)
...
### High-level file writing
    ...
    def copy_file(self, fn, subfolder = False):
        '''
        Copy a relevant file to the directory where the main data file is
        located.
        '''
        filename = self.get_filename()[:-4]
        p, n = os.path.split(fn)
        if subfolder:
            newfn = os.path.join(self.get_dir()+'\\'+str(filename), n)
            d = os.path.dirname(newfn)
            if not os.path.exists(d):
                os.makedirs(d)
        else:
            newfn = os.path.join(self.get_dir(), str(self._timecode)+'_'+n)
        shutil.copyfile(fn, newfn)

    def copy_file2(self, fn, p2=''): #time.strftime('%Y%m%d%H%M%S', time.localtime())):
        '''
        Copy a relevant file to the directory where the main data file is
        located, in a different subdirectory
        give filename and pathname
        '''
        p, n = os.path.split(fn)
        newfn = os.path.join(self.get_dir()+'\\'+str(p2), n)
        d = os.path.dirname(newfn)
        if not os.path.exists(d):
            os.makedirs(d)
        shutil.copyfile(fn, newfn)
