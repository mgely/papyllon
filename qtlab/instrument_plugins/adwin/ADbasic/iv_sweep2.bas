...
DIM loopflag, inoutflag, waitflag, posorneg, twice AS INTEGER
...
EVENT:
	SELECTCASE loopflag
		CASE 0 'sweep to start value
			DAC(PAR_1, counter)
			IF(counter = PAR_2) THEN 'are we at start voltage?
				loopflag = 1
			ELSE
		...
		CASE 1 'sweep and measure simultaneously
			SELECTCASE inoutflag
				CASE 0 'output desired voltage on DAC
					...
					DAC(PAR_1, counter)
					IF (counter = PAR_2)THEN 'are we at the start voltage?
						IF (twice = 1) THEN 'if yes, are we back again or just starting?
							loopflag = 2
						ELSE
							twice = 1
						ENDIF
