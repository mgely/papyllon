...
DIM DATA_3[65536] as integer 
DIM zero,counter1,counter2,counter3,skipcounter,avgcounter,waitcounter as integer
DIM loopflag,inoutflag,waitflag,posorneg as integer '0=sweep to start value, 1=sweep and measure, 2=sweep to zero
DIM totalcurrent,totalcurrentMux2  as LONG
...
SELECTCASE loopflag
	CASE 0 'sweep and measure simultaneously to start
		IF( PAR_10 = 1) THEN loopflag = 3
	  IF( PAR_1>zero ) THEN
    	posorneg=1
    ELSE
	    posorneg=-1
    ENDIF
		SELECTCASE inoutflag
			CASE 0 'output desired voltage on DAC1
				...
				totalcurrent = 0
				totalcurrentMux2 = 0
				counter2 = zero+PAR_3*posorneg
				IF( counter2=PAR_1 ) THEN 'are we at end voltage?
					PAR_5 = READ_TIMER()
					loopflag=1
					skipcounter = PAR_6 - skipcounter + 1
					counter3=PAR_3 
				ELSE
					PAR_3 = PAR_3 + 1
				ENDIF
				DAC(1,zero+PAR_3*posorneg)
			...
			CASE 1 'measure voltage on ADC1
				SELECTCASE waitflag
					...
					CASE 1
						...
						totalcurrentMux2 = totalcurrentMux2 + READADC(2)
						IF( avgcounter = PAR_8 ) THEN
							DATA_1[PAR_7+1] = zero+PAR_3*posorneg
							DATA_2[PAR_7+1] = totalcurrent/PAR_8
							DATA_3[PAR_7+1] = totalcurrentMux2/PAR_8
							PAR_22 = DATA_1[1]
							PAR_23 = DATA_2[1]
							PAR_24 = DATA_3[1]
							PAR_7 = PAR_7 + 1
    ...
    ENDIF
		SELECTCASE inoutflag
			CASE 0 'output desired voltage on DAC1
				...
				totalcurrent = 0
				totalcurrentMux2 = 0
				counter2 = PAR_1+(PAR_3-counter3)*posorneg
				IF( counter2=PAR_2 ) THEN 'are we at end voltage?
					PAR_5 = READ_TIMER()
					loopflag=2
					skipcounter = PAR_6 - skipcounter + 1
					counter3=PAR_3
				ELSE
					PAR_3 = PAR_3 + 1
				ENDIF
				DAC(1,PAR_1+(PAR_3-counter3)*posorneg)
				IF( PAR_3=0 ) THEN 	'read the current time
					PAR_4 = READ_TIMER()
				ENDIF
				IF(skipcounter=PAR_6) THEN inoutflag=1
			...
			CASE 1 'measure voltage on ADC1
				SELECTCASE waitflag
					...
					CASE 1
						...
						totalcurrentMux2 = totalcurrentMux2 + READADC(2)
						IF( avgcounter = PAR_8 ) THEN
							DATA_1[PAR_7+1] = PAR_1+(PAR_3-counter3)*posorneg
							DATA_2[PAR_7+1] = totalcurrent/PAR_8
							DATA_3[PAR_7+1] = totalcurrentMux2/PAR_8
							PAR_22 = DATA_1[1]
							PAR_23 = DATA_2[1]
				...
				ENDSELECT
		ENDSELECT
	CASE 2 'sweep and measure simultaneously back to zero
		IF( PAR_10 = 1) THEN loopflag = 3
		IF( PAR_2 < zero) THEN
    	posorneg=1
    ELSE
    ...
    ENDIF
		SELECTCASE inoutflag
			CASE 0 'output desired voltage on DAC1
				...
				totalcurrent = 0
				totalcurrentMux2 = 0
				counter2 = PAR_2+(PAR_3-counter3)*posorneg
				IF( counter2=zero ) THEN 'are we at end voltage?
					PAR_5 = READ_TIMER()
					loopflag=3
				ELSE
					PAR_3 = PAR_3 + 1
				ENDIF
				DAC(1,PAR_2+(PAR_3-counter3)*posorneg)
				IF( PAR_3=0 ) THEN 	'read the current time
					PAR_4 = READ_TIMER()
				ENDIF
				IF(skipcounter=PAR_6) THEN inoutflag=1
			...
			CASE 1 'measure voltage on ADC1
				SELECTCASE waitflag
					...
					CASE 1
						...
						totalcurrentMux2 = totalcurrentMux2 + READADC(2)
						IF( avgcounter = PAR_8 ) THEN
							DATA_1[PAR_7+1] = PAR_2+(PAR_3-counter3)*posorneg
							DATA_2[PAR_7+1] = totalcurrent/PAR_8
							DATA_3[PAR_7+1] = totalcurrentMux2/PAR_8
							PAR_22 = DATA_1[1]
							PAR_23 = DATA_2[1]
				...
				ENDSELECT
    ENDSELECT
	CASE 3 'sweep back down to zero volts
		DAC(1,counter2)
