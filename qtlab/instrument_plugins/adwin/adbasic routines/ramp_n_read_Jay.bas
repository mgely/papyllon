			...
			CASE 1 'measure voltage on ADC1
				SELECTCASE waitflag
					...
					CASE 1
						START_CONV(1)
						WAIT_EOC(1)
						totalcurren = totalcurren + READADC(1)
						START_CONV(2)
						WAIT_EOC(2)
						totalvoltage = totalvoltage + READADC(2)
						IF( avgcounter = PAR_8 ) THEN
							DATA_1[PAR_7+1] = PAR_1+PAR_3*posorneg
							DATA_2[PAR_7+1] = totalcurren/PAR_8
							DATA_3[PAR_7+1] = totalvoltage/PAR_8
							PAR_22 = DATA_1[1]
							PAR_23 = DATA_2[1]
							PAR_24 = DATA_3[1]
							PAR_7 = PAR_7 + 1
