...
*	 SAVECOMPIL = 0
****************************************
' mega_noise_bias_ramp.bas: ramps voltage on DAC1, recording voltage dependent current on ADC1.
' contains extra data array in order to determine the sigma of the noise
' to compute noise from sample, divide sigma by sqrt(bandwidth)
...
'DATA_2 = (average) <current> array
'DATA_3 = (average) variance <current^2>-<current>^2 array
...
DIM totalcurrent2,dummy as FLOAT
			...
			CASE 1 'measure voltage on ADC1
				SELECTCASE waitflag
					...
					CASE 1
						...
						totalcurrent2 = totalcurrent2 + (READADC(1))^2
						IF( avgcounter = PAR_8 ) THEN
							DATA_1[PAR_7] = PAR_1+PAR_3*posorneg
							DATA_2[PAR_7] = totalcurrent/PAR_8
							'dummy = totalcurrent2/PAR_8
							dummy = sqrt(totalcurrent2/PAR_8 - (DATA_2[PAR_7])*(DATA_2[PAR_7]))
							FPAR_23 = dummy
							DATA_3[PAR_7] = dummy + 32768
							PAR_7 = PAR_7 + 1
							inoutflag=0
							waitflag=0
							skipcounter=1
