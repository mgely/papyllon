...
class ADwin_DAC_sal_inout(Instrument):
    '''
   ...
   TU         address (string)        : GPIB address
        ...
        self.ADwin.Load_Process('C:\qtlab\instrument_plugins\ADwin_DACinout_v2.T91')
        ...
        self.add_parameter('ADC_1',
                            flags=Instrument.FLAG_GETSET,
                            units='V', minval=-10.0, maxval=10.0,
                            type=types.FloatType)
        self.add_parameter('ADC_2',
                            flags=Instrument.FLAG_GETSET,
                            units='V', minval=-10.0, maxval=10.0,
                            type=types.FloatType)
        self.add_parameter('ADC_averaging',
                            flags=Instrument.FLAG_GETSET,
                            units='V', minval=-10.0, maxval=10.0,
                            type=types.FloatType)
...
# --------------------------------------
    ...
    def do_set_ADC_averaging(self,ADC_averaging):
        self.ADwin.Set_Par(9,int(ADC_averaging))
        return clockcycle

    def do_get_ADC_averaging(self):
        globaldelay=self.ADwin.Get_Par(9)
        return globaldelay

    #ADC 1
    def do_get_ADC_1(self):
        self.ADwin.Set_Par(13,1) #set adwin to measuring ADC
        qt.msleep(0.1) #wait a little bit
        bit32=self.ADwin.Get_Par(5)
        return Volt(bit32)

    #ADC 2
    def do_get_ADC_2(self):
        self.ADwin.Set_Par(13,1) #set adwin to measuring ADC
        qt.msleep(0.1) #wait a little bit
        bit32=self.ADwin.Get_Par(6)
        return Volt(bit32)
